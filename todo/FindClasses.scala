...
object FindClasses
{
  ...
  def apply(): Seq[Class[_]] = {
    def findClasses(dir: File, pkg: String): Seq[Class[_]] = {
      dir.listFiles.toSeq.flatMap {
        case f if f.isDirectory => findClasses(f, f.getName)
        case f if f.getName.endsWith(".class") => Seq(Class.forName(pkg + '.' + f.getName.dropRight(6)))
        case _ => Seq.empty
      }
    }
